...
void Renderer::nanogui_init(GLFWwindow* window)
{
    ...
    // //
        ...
        static auto end_point_pos_X = gui_1->addVariable("Current End Position X:", m_bone_animation->end_effector[0]);
        static auto end_point_pos_Y = gui_1->addVariable("Current End Position Y:", m_bone_animation->end_effector[1]);
        static auto end_point_pos_Z = gui_1->addVariable("Current End Position Z:", m_bone_animation->end_effector[2]);
        ...
        glfwSetCursorPosCallback(window,
            ...
            end_point_pos_X->setValue(m_bone_animation->end_effector[0]);
            end_point_pos_Y->setValue(m_bone_animation->end_effector[1]);
            end_point_pos_Z->setValue(m_bone_animation->end_effector[2]);
            bone_rotation_UX->setValue(m_bone_animation->rotate_bones[1][0]);
            bone_rotation_UY->setValue(m_bone_animation->rotate_bones[1][1]);
            bone_rotation_UZ->setValue(m_bone_animation->rotate_bones[1][2]);
            bone_rotation_MX->setValue(m_bone_animation->rotate_bones[2][0]);
...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
    ...
    // Draw three bones
    glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(m_bone_animation->upp_bone_obj_mat_scaled));
    bone_obj->obj_color = m_bone_animation->colors[1];
    draw_object(shader, *bone_obj);
    glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(m_bone_animation->mid_bone_obj_mat_scaled));
    bone_obj->obj_color = m_bone_animation->colors[2];
    draw_object(shader, *bone_obj);
    glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(m_bone_animation->bot_bone_obj_mat_scaled));
