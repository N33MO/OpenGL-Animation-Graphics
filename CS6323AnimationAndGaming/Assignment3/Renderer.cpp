...
void Renderer::nanogui_init(GLFWwindow* window)
{
    ...
    gui_1->addVariable("Catmull-Roll Curve on/off", m_curve->curve_switch);
    
    gui_1->addGroup("Aircraft Animation");
    
    gui_1->addVariable("aircraft is moving", m_aircraft_animation->cb_move);
...
void Renderer::load_models()
{
    ...
    m_curve->calculate_curve(); // set curve to specific points set
	Object curve_object(m_curve->p); // to points set 'p'
	curve_object.m_render_type = RENDER_POINTS;
...
void Renderer::draw_object(Shader& shader, Object& object)
{
	...
	if (object.m_obj_type == RENDER_POINTS) // change 'm_render_type' to 'm_obj_type'
	{
        glPointSize(5.0f);  // thicken the line
