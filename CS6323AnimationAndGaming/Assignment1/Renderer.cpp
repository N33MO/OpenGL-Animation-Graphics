...
float degree_a = 30.0f, degree_b = 45.0f;
const float RADIUS = 5.0f;
float local_cord_X = 5.0f;
float local_cord_Z = 0.0f;
...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	nanogui::ref<nanogui::Window> nanoguiWindow_1 = gui_1->addWindow(Eigen::Vector2i(0, 0), "NanoGUI Control Bar");
...
//    gui_1->addVariable("Z", m_camera->position[2])->setSpinnable(true);
    ...
    gui_1->addGroup("Bunny Transformation");
    gui_1->addVariable("a (local x axis)", degree_a)->setSpinnable(false);
    gui_1->addVariable("b (global y axis)", degree_b)->setSpinnable(false);
    
//    rotate_X_Local = glm::rotate(rotate_X_Local, glm::radians(degree_a), glm::vec3(local_cord_X, 0, local_cord_Z));
//    rotate_Y_Global = glm::rotate(trans * rotate_X_Local * rotate_Y_Global, glm::radians(-degree_b), glm::vec3(0, 0, 0));
...
//    });
    ...
    // buttons in nanogui to modify the object
    gui_1->addButton("Rotate_X_Local", []() {
        theta += degree_a;
    });

    gui_1->addButton("Rotate_Y_Global", []() {
        alpha += degree_b;
    });

    gui_1->addButton("Rotate Both", []() {
        alpha += degree_b;
        theta += degree_a;
    });
...
void Renderer::init()
{
    ...
    // added two lines. currently no use
...
void Renderer::draw_scene(Shader& shader)
{
	...
	for (size_t i = 0; i < obj_list.size(); i++)
	{
		...
		if (obj_list[i].obj_name == "main_object")
		{
            ...
            glm::mat4 rotate_X_Local = glm::mat4(1.0f);
            glm::mat4 rotate_Y_Global = glm::mat4(1.0f);
            glm::mat4 bunny_Position = glm::mat4(1.0f);
            
            // calculate new center position of Bunny
            float local_cord_X = RADIUS * cos(alpha * PI / 180);
            float local_cord_Z = RADIUS * sin(alpha * PI / 180);
            
            // coordinate conversion
            trans = glm::translate(trans, glm::vec3(local_cord_X, 0.0, local_cord_Z));
            rotate_X_Local = glm::rotate(rotate_X_Local, glm::radians(theta), glm::vec3(local_cord_X / RADIUS , 0 ,local_cord_Z / RADIUS));
            rotate_Y_Global = trans * rotate_X_Local * rotate_Y_Global;
            bunny_Position = glm::rotate(rotate_Y_Global, glm::radians(-alpha), glm::vec3(0.0, 1.0, 0.0));
            
            glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(bunny_Position));
