...
void Renderer::draw_bones(Shader& shader, Bone_Animation* m_bone_animation)
{
    ...
    // trans all three bones to the first joint
    upp_bone_obj_mat = glm::translate(upp_bone_obj_mat, m_bone_animation->roottop_position);
    mid_bone_obj_mat = glm::translate(mid_bone_obj_mat, m_bone_animation->roottop_position);
    bot_bone_obj_mat = glm::translate(bot_bone_obj_mat, m_bone_animation->roottop_position);
    
    /*
        // notes for rotate order:
        x -> z -> y ==> 0 -> 2 -> 1 ==> for (int i = 2; i >= 0; i--)
        y -> z -> x ==> 1 -> 2 -> 0 ==> for (int i = 0; i < 3; i++)
    */
    
    // rotate all three bones accord to upp_bone
    for (int i = 2; i >= 0; i--)
    {
        upp_bone_obj_mat = glm::rotate(upp_bone_obj_mat, glm::radians(m_bone_animation->rotate_bones[1][(i+1)%3]), m_bone_animation->rotate_axis[(i+1)%3]);
        mid_bone_obj_mat = glm::rotate(mid_bone_obj_mat, glm::radians(m_bone_animation->rotate_bones[1][(i+1)%3]), m_bone_animation->rotate_axis[(i+1)%3]);
        bot_bone_obj_mat = glm::rotate(bot_bone_obj_mat, glm::radians(m_bone_animation->rotate_bones[1][(i+1)%3]), m_bone_animation->rotate_axis[(i+1)%3]);
    }
    // trans first bone to correct pos
    upp_bone_obj_mat = glm::translate(upp_bone_obj_mat, m_bone_animation->root_upp_trans);
    // trans last two bones to the second joint
    mid_bone_obj_mat = glm::translate(mid_bone_obj_mat, m_bone_animation->root_upptop_trans);
    bot_bone_obj_mat = glm::translate(bot_bone_obj_mat, m_bone_animation->root_upptop_trans);
    // rotate last two bones accord to mid_bone
    for (int i = 2; i >= 0; i--)
    {
        mid_bone_obj_mat = glm::rotate(mid_bone_obj_mat, glm::radians(m_bone_animation->rotate_bones[2][(i+1)%3]), m_bone_animation->rotate_axis[(i+1)%3]);
        bot_bone_obj_mat = glm::rotate(bot_bone_obj_mat, glm::radians(m_bone_animation->rotate_bones[2][(i+1)%3]), m_bone_animation->rotate_axis[(i+1)%3]);
    }
    // trans second bone to correct pos
    mid_bone_obj_mat = glm::translate(mid_bone_obj_mat, m_bone_animation->upptop_mid_trans);
    // trans final bone to the third joint
    bot_bone_obj_mat = glm::translate(bot_bone_obj_mat, m_bone_animation->upptop_midtop_trans);
    // rotate final bone accord to bot_bone
    for (int i = 2; i >= 0; i--)
    {
        bot_bone_obj_mat = glm::rotate(bot_bone_obj_mat, glm::radians(m_bone_animation->rotate_bones[3][(i+1)%3]), m_bone_animation->rotate_axis[(i+1)%3]);
    }
    // trans final bone to correct pos
    bot_bone_obj_mat = glm::translate(bot_bone_obj_mat, m_bone_animation->midtop_bot_trans);
    
    
    upp_bone_obj_mat = glm::scale(upp_bone_obj_mat, m_bone_animation->scale_vector[1]);
    mid_bone_obj_mat = glm::scale(mid_bone_obj_mat, m_bone_animation->scale_vector[2]);
    bot_bone_obj_mat = glm::scale(bot_bone_obj_mat, m_bone_animation->scale_vector[3]);
    
    glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(upp_bone_obj_mat));
    bone_obj->obj_color = m_bone_animation->colors[1];
    draw_object(shader, *bone_obj);
    glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(mid_bone_obj_mat));
    bone_obj->obj_color = m_bone_animation->colors[2];
    draw_object(shader, *bone_obj);
    glUniformMatrix4fv(glGetUniformLocation(shader.program, "model"), 1, GL_FALSE, glm::value_ptr(bot_bone_obj_mat));
