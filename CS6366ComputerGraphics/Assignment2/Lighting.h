...
public:
    ...
    void point_light_rotate(int axis, glm::vec3 objPos)
    {
        ...
        point_light.position[a] = (tmp1 - objPos[a]) * cos(glm::radians(rotate_velocity)) - (tmp2 - objPos[b]) * sin(glm::radians(rotate_velocity)) + objPos[a];
        point_light.position[b] = (tmp2 - objPos[b]) * cos(glm::radians(rotate_velocity)) + (tmp1 - objPos[a]) * sin(glm::radians(rotate_velocity)) + objPos[b];
    }
};
